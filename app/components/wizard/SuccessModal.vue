<template>
  <Teleport to="body">
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <Card class="relative w-full max-w-md shadow-2xl border-primary/20 animate-in zoom-in-95 duration-200"
        @click.stop>
        ...
        <CardContent class="space-y-4">
          ...
          <div class="grid grid-cols-1 gap-2 pt-2">
            <Button variant="secondary" class="w-full" @click="$emit('viewSolicitudes')">
              <ClipboardList class="mr-2 h-4 w-4" />
              Ver mis solicitudes
            </Button>

            <Button v-if="solicitudId" class="w-full bg-primary hover:bg-primary/90" @click="$emit('goToDocumentos')">
              <FileText class="mr-2 h-4 w-4" />
              Continuar a Carga de Documentos
            </Button>

            <Button variant="ghost" class="w-full text-muted-foreground hover:text-foreground" @click="$emit('close')">
              Cerrar
            </Button>
          </div>
        </CardContent>
